...
public class TournamentRunner {
	...
	final static String[] myanacnegotiatorCommand = {"java", "-jar", "agents/MyANACNegotiator.jar", "-log", "log", "-name", "MyANACNegotiator", "-fy", "1905"};
	...
	final static int MOVE_PHASE_DEADLINE = 30;
	final static int RETREAT_PHASE_DEADLINE = 30;
	final static int BUILD_PHASE_DEADLINE = 30;
	
	public static void main(String[] args) throws IOException {
		
		int numberOfGames = 1;				//The number of games this tournament consists of.
		
		int deadlineForMovePhases = MOVE_PHASE_DEADLINE; 	//60 seconds for each SPR and FAL phases
		int deadlineForRetreatPhases = RETREAT_PHASE_DEADLINE;  //30 seconds for each SUM and AUT phases
		int deadlineForBuildPhases = BUILD_PHASE_DEADLINE;  	//30 seconds for each WIN phase
		...
		for(int gameNumber=1; gameNumber<=numberOfGames; gameNumber++){
			...
			for(int i=0; i<7; i++){
				...
				if(i<4){
					...
					name = "MyANACNegotiator " + i;
					command = myanacnegotiatorCommand;
				}
